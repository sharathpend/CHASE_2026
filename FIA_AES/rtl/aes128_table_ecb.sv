...
module aes128_table_ecb (
  ...
  input      [127:0]  state_xor_in, // XOR this with state on 9th round state_reg storage (after round 9 as it gets stored into reg)
...
// Cipher (Encrypt)
  //reg      [31:0]  state_reg[0:3];      // Cipher working registers
  reg      [31:0]  state_reg_0;      // Cipher working registers
  reg      [31:0]  state_reg_1;      // Cipher working registers
  reg      [31:0]  state_reg_2;      // Cipher working registers
  reg      [31:0]  state_reg_3;      // Cipher working registers
...
// --------------------------------------------------------------------------------
  always @( now_state or enc_dec or key_exp or start or start_flag or round_n or key_val ) begin
    case ( now_state )
      ...
      `KEY_EXP    : if ( round_n == 4'd10 ) next_state = `IDLE;         // Key Expansion state_reg
                    else next_state = `KEY_EXP;
      `ROUND_LOOP : if ( round_n == 4'd10 ) next_state = `IDLE;         // Cipher/Invese Cipher state_reg
                    else next_state = `ROUND_LOOP;
...
// ------------------------------------------------------------------------------
  ...
  assign text_out = ( enc_dec == 1'b0 )? { state_reg_0, state_reg_1, state_reg_2, state_reg_3 } : { istate[0], istate[1], istate[2], istate[3] };
...
// ------------------------------------------------------------------------------------------
    ...
    if ( resetn == 1'b0 ) begin
      ...
      if ( enc_dec == 1'b0 ) begin
        if ((( start == 1'b1 ) || ( start_flag == 1'b1 )) && ( round_n == 4'h0 )) begin                                                        // Nr = 0  Add Round Key
          { state_reg_0, state_reg_1, state_reg_2, state_reg_3 } <= add_roundkey0;
        end else if (( now_state == `ROUND_LOOP ) && ( round_n >= 4'd1 ) && ( round_n <= 4'h8 )) begin   // Nr = 1 to Nr = 8  Add round key
          { state_reg_0, state_reg_1, state_reg_2, state_reg_3 } <= add_roundkey;
        end else if (( now_state == `ROUND_LOOP ) && ( round_n == 4'h9 )) begin   // Nr = 9  Add round key
          { state_reg_0, state_reg_1, state_reg_2, state_reg_3 } <= add_roundkey ^ state_xor_in;
        end else if (( now_state == `ROUND_LOOP ) && ( round_n == 4'd10 )) begin   // Nr = 10
          { state_reg_0, state_reg_1, state_reg_2, state_reg_3 } <= cipher_text;
        end else if (( now_state == `IDLE ) && ( text_val == 1'd1 )) begin   //reset after finish
      		{ state_reg_0, state_reg_1, state_reg_2, state_reg_3 } <= 128'b0;
	      end else begin
          { state_reg_0, state_reg_1, state_reg_2, state_reg_3 } <= { state_reg_0, state_reg_1, state_reg_2, state_reg_3 };
        end
      end else begin
        { state_reg_0, state_reg_1, state_reg_2, state_reg_3 } <= { state_reg_0, state_reg_1, state_reg_2, state_reg_3 };
      end
  ...
  // ------------------------------------------------------------------------------------------------------------
  subbytes SubBytes (
    .a00( state_reg_0[31:24] ), .a10( state_reg_0[23:16] ), .a20( state_reg_0[15:8] ), .a30( state_reg_0[7:0] ),
    .a01( state_reg_1[31:24] ), .a11( state_reg_1[23:16] ), .a21( state_reg_1[15:8] ), .a31( state_reg_1[7:0] ),
    .a02( state_reg_2[31:24] ), .a12( state_reg_2[23:16] ), .a22( state_reg_2[15:8] ), .a32( state_reg_2[7:0] ),
    .a03( state_reg_3[31:24] ), .a13( state_reg_3[23:16] ), .a23( state_reg_3[15:8] ), .a33( state_reg_3[7:0] ),
...
// Invese Cipher
// ------------------------------------------------------------------------------------------------------------
  // Invese Cipher state_reg registers
  always @(posedge clock or negedge resetn) begin
    if ( resetn == 1'b0 ) begin
